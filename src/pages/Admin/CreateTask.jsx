...
import AddAttachMentsInput from '../../components/charts/AddAttachMentsInput';
...
const CreateTask = () => {
    ...
    const createTask = async () => {
        try {
            ...
            console.error("Error for create a new Task ", error);
            setLoading(false);
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async () => {
        ...
        if (taskData.todoChecklist?.length === 0) {
            setError("Add atLatest one todo task");
            return;
        }
        if (taskId) {
            updateTask();
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 mt-4 ">
                    <div className="form-card col-span-3">
                        ...
                        <div className="mt-4">
                            ...
                            <input type="text" placeholder='Create App UI' className='form-input'
                                value={taskData.title}
                                onChange={({ target }) => handleValueChange("title", target.value)} />
                        </div>
                        <div className="mt-3">
                            <label className='text-xs font-medium text-slate-600 ' >Description</label>
                            <textarea placeholder='Description task'
